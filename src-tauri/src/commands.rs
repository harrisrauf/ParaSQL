...
pub async fn get_all_rows(state: State<'_, AppState>) -> Result<Vec<RowData>, String> {
    with_engine(state, move |engine| engine.get_all_rows()).await
}
...
pub async fn get_page(
    ...
    with_engine(state, move |engine| engine.get_page(offset, limit)).await
}
...
pub async fn delete_rows(row_ids: Vec<u64>, state: State<'_, AppState>) -> Result<(), String> {
    with_engine_mut(state, move |engine| engine.delete_rows(&row_ids)).await
}

#[tauri::command]
pub async fn insert_row(state: State<'_, AppState>) -> Result<QueryResult, String> {
    with_engine_mut(state, move |engine| engine.insert_row()).await
}

#[tauri::command]
pub async fn add_column(name: String, dtype: String, state: State<'_, AppState>) -> Result<(), String> {
    with_engine_mut(state, move |engine| engine.add_column(&name, &dtype)).await
}

#[tauri::command]
pub async fn drop_column(name: String, state: State<'_, AppState>) -> Result<(), String> {
    with_engine_mut(state, move |engine| engine.drop_column(&name)).await
}
...
pub async fn rename_column(
    ...
    with_engine_mut(state, move |engine| engine.rename_column(&old_name, &new_name)).await
}
...
pub async fn sort_by(
    ...
    with_engine(state, move |engine| engine.sort_by(&col_name, ascending)).await
}
...
pub async fn undo(state: State<'_, AppState>) -> Result<(), String> {
    with_engine_mut(state, move |engine| engine.undo()).await
}

#[tauri::command]
pub async fn redo(state: State<'_, AppState>) -> Result<(), String> {
    with_engine_mut(state, move |engine| engine.redo()).await
}

#[tauri::command]
pub async fn can_undo(state: State<'_, AppState>) -> Result<bool, String> {
    with_engine(state, move |engine| Ok(engine.can_undo())).await
}

#[tauri::command]
pub async fn can_redo(state: State<'_, AppState>) -> Result<bool, String> {
    with_engine(state, move |engine| Ok(engine.can_redo())).await
}

#[tauri::command]
pub async fn export_json(path: String, state: State<'_, AppState>) -> Result<(), String> {
    with_engine_mut(state, move |engine| engine.export_json(&path)).await
}

#[tauri::command]
pub async fn export_csv(path: String, state: State<'_, AppState>) -> Result<(), String> {
    with_engine_mut(state, move |engine| engine.export_csv(&path)).await
}

#[tauri::command]
pub async fn export_excel(path: String, state: State<'_, AppState>) -> Result<(), String> {
    with_engine_mut(state, move |engine| engine.export_excel(&path)).await
}
